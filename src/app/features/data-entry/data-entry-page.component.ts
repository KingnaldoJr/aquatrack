...
import {
  ...
  WaterUsageFormValue,
} from './components/water-usage-form/water-usage-form.component';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, WaterUsageFormComponent, MatSnackBarModule],
...
export class DataEntryPageComponent implements OnInit {
  ...
  onUsageSubmit(formData: WaterUsageFormValue): void {
    if (!formData.houseId || !formData.condominiumId) {
      this.snackBar.open('Please select both condominium and house.', 'Close', {
        duration: 3000,
      });
      return;
    }
